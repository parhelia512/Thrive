...
#pragma warning restore CA2213
        ...
        bool isLastGeneration = false)
    {
        ...
        node.Pressed += () => { OnTreeNodeSelected(node); };
    ...
    private void UpdateTreeNodeSizeAndPosition()
    {
        ...
        foreach (var node in speciesNodes.Values.SelectMany(speciesNodeList => speciesNodeList))
        {
            ...
            // Size needs to be adjusted explicitly even when SizeFlag set to ShrinkEnd.
    ...
    private void TimelineDraw()
    {
        ...
        for (int i = firstDrawnGeneration; i <= lastDrawnGeneration; i += increment)
        {
            ...
            timeline.DrawString(latoSmallRegular, new Vector2(Mathf.Clamp(x - size.X / 2, 0, Size.X - size.X),
                TIMELINE_AXIS_Y + TIMELINE_MARK_LENGTH * 2 + size.Y), localizedText, modulate: Colors.Cyan);
        }
    }
    ...
    private void GUIInput(InputEvent @event, bool horizontalOnly)
    {
        ...
        {
            dragging = buttonEvent.ButtonMask.HasFlag(MouseButtonMask.Left) ||
                buttonEvent.ButtonMask.HasFlag(MouseButtonMask.Right);
            if (dragging)
                lastMousePosition = buttonEvent.Position;

            if (buttonEvent is { Pressed: true, ButtonIndex: MouseButton.WheelDown or MouseButton.WheelUp })
            {
    ...
    {
        // TreeSize may be less than Size, so the later Min and Max is not merged into Clamp.
            ...
            if (lastNode.LastGeneration)
            {
                tree.DrawString(latoSmallItalic,
                    new Vector2(lastNode.Position.X + sizeFactor * TreeNodeSize.X + speciesNameOffset,
                        lastNode.Center.Y), speciesNames[pair.Key], modulate: Colors.DarkRed);
            }
            else
            {
                tree.DrawString(latoSmallItalic,
