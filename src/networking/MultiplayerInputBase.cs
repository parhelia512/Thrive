...
public abstract class MultiplayerInputBase : PlayerInputBase
{
    ...
    public void NetworkTick(float delta)
    {
        ...
        if (NetworkManager.Instance.IsClient &&
            NetworkManager.Instance.LocalPlayer?.Status == NetworkPlayerStatus.Active)
        {
            // Send redundant inputs

            var batch = new NetworkInputBatch
            {
                StartTick = MultiplayerStage.LastAckedInputTick,
            };

            // TODO: this seems to send too many redundant inputs at times
            for (uint tick = MultiplayerStage.LastAckedInputTick; tick <= MultiplayerStage.TickCount; ++tick)
            {
