...
public class PackedBytesBuffer
{
    ...
    ///   </para>
    /// </remarks>
    public void Write(PackedBytesBuffer buffer)
    {
        Write(buffer.Length);
        Write(buffer.Data);
    }
    ...
    /// <summary>
    ///   Reads a byte array wrapped as <see cref="PackedBytesBuffer"/> from the buffer and advances the current
    ...
    ///   </para>
    /// </remarks>
    public PackedBytesBuffer ReadBuffer()
    {
        var size = ReadInt32();
        return new PackedBytesBuffer(ReadBytes(size));
    }
