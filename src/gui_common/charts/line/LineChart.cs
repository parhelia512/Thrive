...
#pragma warning restore CA2213
        ...
        int expandedXTicks = 0, int expandedYTicks = 0)
    {
        ...
        foreach (var data in dataSets)
        {
            ...
            dataLine.TreeExited += () => { RemoveDataLineTooltipOnDeletion(dataLine); };
    ...
    public void AddIconLegend(Texture2D icon, string name, float size = 15)
    {
        ...
        {
            Texture = icon,
            ExpandMode = TextureRect.ExpandModeEnum.FitHeight | TextureRect.ExpandModeEnum.FitWidth,
    ...
    private void DrawOrdinateLines()
    {
        foreach (Control tick in verticalLabelsContainer.GetChildren())
        {
            drawArea.DrawTextureRect(hLineTexture, new Rect2(new Vector2(
                    0, tick.Position.Y + (tick.Size.Y / 2)), drawArea.Size.X, 1), false,
            ...
            foreach (var point in points)
            {
                ...
                if (index < dataLine.Points.Length)
                {
                    dataLine.InterpolatePointPosition(
                        index, point.Position + ((Control)point).Size / 2, point.Coordinate);
                }
                else
                {
    ...
    private void UpdateLineColliders(string datasetName, DataPoint firstPoint, DataPoint secondPoint)
    {
        ...
        mouseCollider.Scale = new Vector2(
            firstPoint.Coordinate.DistanceTo(secondPoint.Coordinate) - ((Control)firstPoint).Size.X,
    ...
    private void DrawErrorText(string error)
    {
        ...
        var position = new Vector2(
            Mathf.Round((drawArea.Size.X - font.GetStringSize(error).X) / 2),
    ...
    public partial class DatasetsIconLegend : RefCounted, IDataSetsLegend
    {
        ...
        public virtual Control CreateLegend(DataSetDictionary datasets, string? title)
        {
            ...
            foreach (var data in datasets)
            {
                ...
                icon.Toggled += t => { OnIconLegendToggled(t, icon); };
