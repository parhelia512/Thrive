...
public static class KeyPromptHelper
{
    ...
    /// <summary>
    ///   Button theme for keyboard, valid values: "Dark", "Light3D"
    /// </summary>
    ...
    ///   A tuple of path to the icon for the action and potentially an overlay image to be drawn on top
    /// </returns>
    public static (string Primary, string? Overlay) GetPathForAction(Godot.Collections.Array<InputEvent> actionList)
    {
        ...
        foreach (var action in actionList)
        {
            switch (InputMethod)
            {
                ...
                case ActiveInputMethod.Controller:
                {
                    ...
                    {
                        return (GetPathForControllerButton((JoyButton)joypadButton.ButtonIndex), null);
                    }
    ...
    private static string GetPlayStationControllerButton(string type, JoyButton button)
    {
        ...
        switch (button)
        {
            case JoyButton.X:
                ...
                break;
            /*
            case JoyButton.SonyCircle:
                buttonName = "Circle";
                break;
            case JoyButton.SonySquare:
                buttonName = "Square";
                break;
            case JoyButton.SonyTriangle:
                buttonName = "Triangle";
                break;
            case JoyButton.L:
                buttonName = "L1";
